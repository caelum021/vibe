...
+ 코드베이스를 위한 옵시디언. 프로젝트 내 마크다운 문서를 1급 시민으로 대우한다.
옵시디언의 킬러 피처는 렌더링이나 검색이 아니라 **그래프 뷰와 백링크**다. 설계문서는 본질적으로 의존성 그래프이기 때문이다. `project-brief` → `dev-plan` → `phase-work-order`처럼 문서 간 관계가 설계의 구조 그 자체다. 지금 Vibe는 문서를 플랫한 리스트로 보여주지만, v1.1.0에서는 그 관계를 드러낸다.

### 우선순위 높음 (난이도 낮음, 효과 높음)

**백링크 표시**
문서 뷰어 하단에 "이 문서를 참조하는 문서" 목록 표시. 대시보드 로드 시 전체 md를 스캔해 역방향 인덱스 구축, 파일 변경 감지로 업데이트.

**문서 간 링크 탐색**
`[텍스트](./foo.md)` 클릭 시 뷰어에서 해당 파일 열기. 외부 URL과 내부 링크 시각적 구분.

**끊어진 링크 / 고아 문서 감지**
- 끊어진 링크: 존재하지 않는 파일을 가리키는 링크 (문서 리팩토링 후 흔한 문제)
- 고아 문서: 어디서도 참조되지 않는 md (버려진 설계문서 후보)
- 대시보드에 경고 배지로 표시
...
**코드-문서 양방향 링크**
md 문서가 `src/components/Chart.tsx`를 언급하면 링크로 인식. 코드 파일에서 "이 파일을 참조하는 설계문서" 역추적. 옵시디언은 순수 노트 도구라 불가능하지만, vibe는 프로젝트 전체를 보기 때문에 가능하다.
