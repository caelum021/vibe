...
코드베이스를 위한 옵시디언. 프로젝트 내 md 문서 간 관계(링크 그래프, 백링크)를 1급 시민으로 다룬다. 옵시디언의 킬러 피처는 렌더링이 아니라 **그래프와 백링크**라는 판단 — 설계문서는 본질적으로 의존성 그래프이기 때문. `project-brief` → `dev-plan` → `phase-work-order`처럼 문서 간 관계가 설계의 구조 그 자체다.
